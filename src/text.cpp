#include <SDL2/SDL.h>
#include <SDL2/SDL_image.h>
#include <SDL2/SDL_ttf.h>
#include <iostream>
#include <string>
#include <vector>
#include <array>

#include "Entity.hpp"
#include "Text.hpp"

Text::Text(std::string texText, SDL_Color textColour, std::string fontFile, unsigned int fontSize, std::array<int, 4> src, std::array<int, 2> dst, SDL_Renderer* renderer)
    :texText(texText), textColour(textColour), fontFile(fontFile), fontSize(fontSize)
{
    this->renderer = renderer;

    std::cout << fontFile << std::endl;

    this->font = TTF_OpenFont(this->fontFile.c_str(), this->fontSize);
    if (this->font == nullptr)
        std::cout << "(Text) Failed to load font! SDL_ttf Error: " << TTF_GetError() << std::endl;
    else
    {
        SDL_Surface* textSurface = TTF_RenderText_Solid(this->font, this->texText.c_str(), this->textColour);
        if (textSurface == nullptr)
            std::cout << "(Text) Unable to render text surface! SDL_ttf Error: " << TTF_GetError() << std::endl;
        else
    {
        this->tex = SDL_CreateTextureFromSurface(this->renderer, textSurface);
        if (this->tex == nullptr)
            std::cout << "(Text) Unable to create texture from rendered text! SDL Error: " << SDL_GetError() << std::endl;
        else
        {
            this->texWidth = textSurface->w;
            this->texHeight = textSurface->h;
        }

        SDL_FreeSurface(textSurface);
    }
    }

    // this->src->x = src[0];
    // this->src->y = src[1];
    // this->src->w = src[2];
    // this->src->h = src[3];
    this->src = nullptr;

    this->dst->x = dst[0];
    this->dst->y = dst[1];
    this->dst->w = this->texWidth;
    this->dst->h = this->texHeight;
    // this->dst->w = src[2];
    // this->dst->h = src[3];
}

void Text::closeFont()
{
    TTF_CloseFont(this->font);
    this->close();
}

TTF_Font* Text::getFont()
 {
     return this->font;
 }
